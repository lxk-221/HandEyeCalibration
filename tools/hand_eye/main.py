...
# 用哪个 arm / camera 由 config.yaml 决定 (懒加载, 见 tools/_hardware.py)。
# 实例化参数 (如 robot_ip) 仍由下方 argparse 从命令行传入。
from .._hardware import get_arm_class, get_camera_class
...
def main(argv=None):
    ...
    ArmImpl = get_arm_class()
    CameraImpl = get_camera_class()
    if ArmImpl is None:
        raise RuntimeError("config.yaml 未配置 arm (或为 null), 无法启动手眼标定")
    if CameraImpl is None:
        raise RuntimeError("config.yaml 未配置 camera (或为 null), 无法启动手眼标定")

    os.makedirs(OUT_DIR, exist_ok=True)
    print(f"=== eye-in-hand 手眼标定 (arm={ArmImpl.__name__}, camera={CameraImpl.__name__}) ===")
